...
class L2Penalty:
    ...
    This is called as:
        ...
        g = l1pen(1, x);   - to get the gradient value nabla L1pen(x) at x
        H = l1pen(2, x);   - to get the Hessian value nabla^2 L1pen(x) at x
    """

    def __init__(self, nl_file):
        global nlp
        self.fact = default_fact
        nlp = NLP.NonlinearProblem(nl_file)
    ...
    def l2pen(self, order, x):
        ...
        if order == 0:
            # L2pen(x) = nlp.f(x)
            # + 0.5*sum_i(max(c_i(x)-bl_i, 0)^2)
            # + 0.5*sum_i(max(bu_i-c_i(x), 0)^2)
            ...
            # hess = nabla^2 f(x) + c*h(x)nabla^2 h(x)+c*nabla h(x)'*nabla h(x)

            lam = np.zeros(nlp.n_con)
            c = nlp.cons(x)
            H0 = nlp.hess(x, lam)
            J0 = nlp.jac(x)
